...
def install(runtime, environment, docs):
    ...
    if docs:
        commands = [
            "edm install -e {environment} enthought_sphinx_theme --yes"
        ]
        execute(commands, parameters)
...
def test(runtime, environment, integration):
    ...
    commands = [
        "edm run -e {environment} -- coverage run -p -m nose.core -v codetools "
        "--nologcapture"
    ]

    # We run in a tempdir to avoid accidentally picking up wrong codetools
...
def get_parameters(runtime, environment):
    ...
    if environment is None:
        parameters['environment'] = 'codetools-test-{runtime}'.format(
            **parameters
        )
    return parameters
